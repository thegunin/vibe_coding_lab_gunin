...
class Database:
    ...
    def add_feedback_entry(self, user_id: int, message: str) -> int:
        """Persist feedback message from employee and return entry id."""
        with self.connect() as conn:
            cursor = conn.execute(
                """
    ...
    def list_feedback_entries(self, limit: int = 50, offset: int = 0) -> list[sqlite3.Row]:
        """Return recent feedback entries for manager review."""
        with self.connect() as conn:
            rows = conn.execute(
                """
                SELECT f.id,
